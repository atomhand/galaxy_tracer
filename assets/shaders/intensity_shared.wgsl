...
f32 get_height_modulation(height : f32) -> f32 {
...
f32 get_radial_intensity(distance : f32, r0 : f32) -> f32 {
...
f32 all_arms_modifier(distance : f32, p : vec3<f32>, angular_offset : f32) -> f32 {
    var v = 0.0;
    num_arms = 2;// 0 to 4
    for(var i = 0; i<4; i++) {
        if i >= num_arms { break; }
        v = max(v,get_arm_modifier(distance,p,angular_offset,i));
    }
    return v;
}

f32 get_intensity_coefficient(p : vec3<f32>, weight : f32, is_arm : bool) -> f32 {
    ...
    let r0 = 0.5;
    let inner = 0.5; // Not actually sure what this is referred to as in 
