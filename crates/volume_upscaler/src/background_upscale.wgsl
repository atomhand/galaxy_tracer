...
fn fragment(in: FullscreenVertexOutput) -> Output {   
    ...
    let world_pos = view.world_from_clip * vec4<f32>(clip_pos,0.0,1.0);
    let prev_clip_pos = (previous_view.clip_from_world * world_pos).xy;

    let velocity = (clip_pos.xy - prev_clip_pos) * vec2(0.5, -0.5); // this is the ndc_to_uv conversion, but we can drop some terms that cancel out

    let old_uv = in.uv - velocity;//ndc_to_uv(prev_clip_pos);

    var force_new_sample =any(saturate(old_uv) != old_uv);
